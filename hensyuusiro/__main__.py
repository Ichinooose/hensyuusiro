...
async def voiceevent(event: hikari.VoiceStateUpdateEvent) -> None:
    try:
        if event.state.channel_id == int(voice_channel_id) or event.old_state.channel_id == int(voice_channel_id):
            ...
            if event.old_state is None:
                ...
                await (await client.rest.fetch_channel(CHANNEL_ID)).send(embed=embed)
...
async def video_time_start(ctx: lightbulb.Context) -> None:
    try:
        if ctx.get_guild().get_voice_state(ctx.user).channel_id == voice_channel_id:
...
async def video_time_end(ctx: lightbulb.Context,) -> None:
    ...
    with open(file, "r") as yml:
        config = yaml.safe_load(yml)
    if int(config["Start_Video_Time"][0:2]) <= int(ctx.options.end[0:2]):
        try:
            if ctx.get_guild().get_voice_state(ctx.user).channel_id == voice_channel_id:
        ...
        embed = hikari.Embed(color="b22222")
        embed.add_field(name="実行されませんでした。", value="限界値に達しているかもしくは開始コマンド入力がされてません。")
        await (await client.rest.fetch_channel(CHANNEL_ID)).send(embed=embed)
